...
curl -s -H "Content-Type: application/json; charset=utf-8" -X PUT "$ADDRESS/book/_doc/1?pretty" -d'{
  "commodity_type": "图书",
...
echo
curl -s -H "Content-Type: application/json; charset=utf-8" -X PUT "$ADDRESS/book/_doc/3?pretty" -d'{
  "commodity_type": "图书",
  "commodity_name": "Go语言高级编程",
  "describe": "Go语言进阶实战，CGO编程web编程树，云计算云存储区块链时代重要编程语言，满足Gopher好奇心的Go语言进阶读物",
  "book_type": "IT技术",
...
echo
curl -s -H "Content-Type: application/json; charset=utf-8" -X PUT "$ADDRESS/book/_doc/13?pretty" -d'{
  "commodity_type": "图书",
